...
class test{
    ...
    void solve(){
        for(int i=0;i<old_set_size;i++){
            int l=sets[i].size();
            for(int i=1;i<=l;i++)output+=to_string(i)+"\n";
        }
        for(operation A:operations){
            vector<int> temp;
            switch(A.type){
...
int main(){
    vector<json> tests;
    int T=1;
    string outf="../.cph/.Main.cpp_1943e3636038f3f4f355cc9661321e0d.prob";
    for(;T--;){
        test U;
        U.out["id"]=T;
        tests.push_back(U.out);
        cout<<"T: "<<T<<endl;
    }
    ifstream fin;
    fin.open(outf);
